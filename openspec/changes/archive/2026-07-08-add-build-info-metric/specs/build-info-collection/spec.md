...
If the `ver` command fails, times out, or returns an empty, whitespace-only,
non-printable, or oversized (over 256 bytes) response, the exporter SHALL
omit `meshcoretel_build_info` for that scrape and SHALL NOT alter
`meshcoretel_up` or fail the scrape because of it. The failure SHALL be
logged at warning level without including the password or session token.
...
When build-info collection is enabled, the exporter's existing per-scrape
timeout budget (login + stats, honoring
`X-Prometheus-Scrape-Timeout-Seconds`) SHALL also cover the `ver` command,
rather than introducing a second independent timeout.
